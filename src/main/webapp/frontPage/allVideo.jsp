  		...
  		<div class="col-sm-8 col-sm-offset-2 col-md-11 col-md-offset-1 main">
					...
					<tbody>
						<c:if test="${!empty videoList}">
							...
							<tr>  
                                ...
                                <td><button><a href="${APP_PATH}/download?filename=${video.path}">下载</a></button></td>
				...
				<div class="col-md-6">
						<nav aria-label="Page navigation">
						  <ul class="pagination">
						  	<li><a href="${APP_PATH}/getAllVideo?pn=1">首页</a></li>
						  	<c:if test="${videoList.hasPreviousPage}">
							    <li>
							      <a href="${APP_PATH}/getAllVideo?pn=${videoList.pageNum-1}" aria-label="Previous">
						    ...
						    <c:forEach items="${videoList.navigatepageNums}" var="navigatepageNums">
						    	...
						    	<c:if test="${navigatepageNums!=videoList.pageNum }">
						    		<li><a href="${APP_PATH}/getAllVideo?pn=${navigatepageNums}">${navigatepageNums }</a></li>
						    	</c:if>
						    </c:forEach>
						    <c:if test="${videoList.hasNextPage}">
							    <li>
							      <a href="${APP_PATH}/getAllVideo?pn=${videoList.pageNum+1}" aria-label="Next">
							        <span aria-hidden="true">&raquo;</span>
							      </a>
							    </li>
						    </c:if>
						    <li><a href="${APP_PATH}/getAllVideo?pn=${videoList.pages }">末页</a></li>
						  </ul>
						</nav>			
					</div>		
			</div>
